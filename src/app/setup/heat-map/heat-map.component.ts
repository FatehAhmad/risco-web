...
import { HeatMapModel } from '../../shared/models/heat-map';
import { HeatMapLatLng } from '../../shared/models/heat-map-latlng';
import { CommentPostModel } from 'src/app/shared/models/comment-post';
...
export class HeatMapComponent implements OnInit {
  HashTag: any;
  objHeatMap: HeatMapModel = new HeatMapModel();
  map: any;
  heatmap: any;
  heatmap2: any;
  heatmap3: any;
  ...
  constructor(private objPostService: PostService, private objRoute: ActivatedRoute, public objDataService: DataService) {
  }
  ...
  // function getPointsHighRisk() {
  //   ;
  //   var i;
  //   var highRiskPts = [];

  //   for(i=0;i<that.objHeatMap.High.length;i++)
  //   {
  //     highRiskPts.push(new google.maps.LatLng(that.objHeatMap.High[i].Latitude, that.objHeatMap.High[i].Longitude))
  //   }
  //   return highRiskPts;
  // }
  ...
  initializeScripts() {
    ...
    // var initMap;
    function initMap() {
      that.map = new google.maps.Map(document.getElementById('maps'), {
        zoom: 1,
        center: new google.maps.LatLng(that.objHeatMap.High[0].Latitude, that.objHeatMap.High[0].Longitude),
        mapTypeId: 'roadmap'
      });

      //map working
      // var data = getPointsHighRisk();
      ...
      that.heatmap = new google.maps.visualization.HeatmapLayer({
        data: getPointsHighRisk(),
        map: that.map
      });
      highGradient();

      // that.heatmap = new google.maps.visualization.HeatmapLayer({
      //   data: getPointsMediumRisk(),
      //   map: that.map
      // });
      // mediumGradient();

      // that.heatmap = new google.maps.visualization.HeatmapLayer({
      //   data: getPointsLowRisk(),
      //   map: that.map
      // });
      // lowGradient();
    ...
    function getPointsHighRisk() {
      ...
      for (i = 0; i < that.objHeatMap.High.length; i++) {
        highRiskPts.push(new google.maps.LatLng(that.objHeatMap.High[i].Latitude, that.objHeatMap.High[i].Longitude))
        bounds.extend(new google.maps.LatLng(that.objHeatMap.High[i].Latitude, that.objHeatMap.High[i].Longitude));

        marker = new google.maps.Marker({
          position: new google.maps.LatLng(that.objHeatMap.High[i].Latitude, that.objHeatMap.High[i].Longitude),
          map: that.map,
          // icon: '../../../',            //add icon here later
          // title: 'Hello World!',
          postId: that.objHeatMap.High[i].Id,
          // content: '<div class="richmarker-wrapper"><p></p></div>'
        });
        marker.addListener('click', function() {

          console.log(that.objHeatMap.High[i]);

          // that.commentClicked(this.postId, undefined);


          // this will trigger the commentCliced method in the header component through ngOnInit
          that.objDataService.notifyOther(this.postId);
    ...
    function getPointsMediumRisk() {
      var i;
      var MediumRiskPts = [];

      for (i = 0; i < that.objHeatMap.Medium.length; i++) {
        MediumRiskPts.push(new google.maps.LatLng(that.objHeatMap.Medium[i].Latitude, that.objHeatMap.Medium[i].Longitude))
        bounds.extend(new google.maps.LatLng(that.objHeatMap.Medium[i].Latitude, that.objHeatMap.Medium[i].Longitude));
      }
      return MediumRiskPts;
    }

    function getPointsLowRisk() {
      var i;
      var LowRiskPts = [];

      for (i = 0; i < that.objHeatMap.Low.length; i++) {
        LowRiskPts.push(new google.maps.LatLng(that.objHeatMap.Low[i].Latitude, that.objHeatMap.Low[i].Longitude))
        bounds.extend(new google.maps.LatLng(that.objHeatMap.Low[i].Latitude, that.objHeatMap.Low[i].Longitude));

      }
      return LowRiskPts;
    }

    function highGradient() {
      ...
      that.heatmap.set('gradient', that.heatmap.get('gradient') ? null : gradient);
    }

    function mediumGradient() {
      var gradient = [
        'rgba(0, 255, 255, 0)',
        'rgba(0, 0, 255, 1)',
        'rgba(0, 0, 255, 1)',
    ...
    $('#toggle').on('click', function () {
      that.heatmap.setMap(that.heatmap.getMap() ? null : that.map);
    });

    // $('#gradient').on('click', ch1());

    $('#radius').on('click', function () {
      that.heatmap.set('radius', that.heatmap.get('radius') ? null : 20);
    });

    $('#opacity').on('click', function () {
      that.heatmap.set('opacity', that.heatmap.get('opacity') ? null : 0.2);
    });
  ...
  ngOnInit() {
    ...
    this.objRoute.params.subscribe(paramsId => {
      ...
      this.HashTag = paramsId["HashTag"];
    });


    this.objPostService.getLocationsForHeatMap(this.HashTag)
      .subscribe(res => {
        if (res.StatusCode == 200) {
          this.objHeatMap.Low = res.Result.Low;
          this.objHeatMap.Medium = res.Result.Medium;
          this.objHeatMap.High = res.Result.High;
