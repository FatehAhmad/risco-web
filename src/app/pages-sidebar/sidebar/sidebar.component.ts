...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.groupId == undefined) {
        ...
        if (user) {
          ...
          this.objUserService.getUserData().subscribe(res => {
            if (res.StatusCode == 200) {
              ...
              for (
                i = 0;
                i < this.objAccountSetting.BasketSettings.Interests.length;
                i++
              ) {
                if (
                  this.objAccountSetting.BasketSettings.Interests[i].Checked ==
                  true
                ) {
          ...
          this.objPostService.searchTrends(1).subscribe(res => {
            if (res.StatusCode == 200) {
              this.objTrendsIncident = res.Result;
            }
          });
        }
      }
      ...
      if (this.userId != undefined) {
        this.objUserService.getUserById(this.userId).subscribe(res => {
          if (res.StatusCode == 200) {
            this.objAccountSetting = res.Result;
            this.aboutMeContent = this.objAccountSetting.AboutMe;
        ...
        this.objPostService.searchTrends(0).subscribe(res => {
          if (res.StatusCode == 200) {
            this.objTrendsIncident = res.Result;
          }
        });
    ...
    if (ext == "jpg" || ext == "png" || ext == "jpeg") {
      ...
      reader.onload = function(e) {
        ...
        that.objGroupService
          .updateGroupImage(file, that.Group_Id)
          .subscribe(res => {
            if (res.StatusCode == 200) {
            }
          });
